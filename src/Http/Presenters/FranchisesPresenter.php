...
class FranchisesPresenter extends Presenter
{
    /**
    ...
    {
        return $this->table->of('franchise', function (TableGrid $table) use ($model) {
            ...
            $table->column('action')
                ...
                ->value(function ($row) {
                    ...
                    if (! in_array((int) $row->id, $roles)) {
                        $html[] = app('html')->link(
                            handles("blupl::franchise/registration/{$row->id}/delete", ['csrf' => true]),
    ...
    {
        return $this->form->of('franchises.registration', function (FormGrid $form) use ($model) {
            $form->resource($this, 'blupl/franchise::registration', $model);
